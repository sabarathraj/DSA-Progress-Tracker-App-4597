...
const Settings: React.FC = () => {
  ...
  const [profileData, setProfileData] = useState<{ full_name: string; daily_goal: number }>({
  ...
  const handleResetAccount = async () => {
    ...
    try {
      ...
      await dbHelpers.resetUserData(user.id);
      await loadUserProfile(user.id);
      if (window.location.pathname !== '/dashboard') {
        window.location.href = '/dashboard';
      }
      toast.success('Account reset! All your progress and data have been cleared.');
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full px-4 py-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 dark:bg-red-900 dark:text-red-200 dark:hover:bg-red-800 transition-colors flex items-center justify-center space-x-2 border border-red-200 dark:border-red-700 mt-2 disabled:opacity-60"
                disabled={resetLoading}
              >
                <SafeIcon icon={FiIcons.FiTarget} className="w-4 h-4" />
                <span>Reset Account</span>
                {resetLoading && <span className="ml-2 animate-spin">⏳</span>}
              </button>
            </div>
          </motion.div>
        </motion.div>
      ...
      {showResetModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-8 max-w-md w-full border border-gray-200 dark:border-gray-700">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Reset Account?</h2>
            <p className="text-gray-700 dark:text-gray-300 mb-4">This will <span className="font-semibold text-red-600">permanently delete all your progress, problems, code, badges, and stats</span> and reset your account to a fresh state. This cannot be undone. Are you sure?</p>
