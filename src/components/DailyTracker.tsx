...
const DailyTracker = () => {
  ...
  const safeTodayProgress = todayProgress ?? {
    solved: 0,
    revised: 0,
    goal: 1,
    revisionGoal: 0,
    achieved: false,
    revisionAchieved: false,
    xpEarned: 0,
    studyTime: 0,
    focusAreas: [],
    notes: ''
  };
  const progressPercentage = Math.min((safeTodayProgress.solved / safeTodayProgress.goal) * 100, 100);
  const revisionPercentage = safeTodayProgress.revisionGoal > 0 
    ? Math.min((safeTodayProgress.revised / safeTodayProgress.revisionGoal) * 100, 100) 
    : 0;
    ...
    <Card className="p-6 shadow-sm border border-gray-200 dark:border-gray-700" padding="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          {safeTodayProgress.revisionAchieved && safeTodayProgress.revisionGoal > 0 && (
            <motion.div
      ...
      <div className="flex items-center justify-center space-x-8 mb-6">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                <div className="text-lg font-bold text-gray-900 dark:text-white">
                  {safeTodayProgress.solved}
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  of {safeTodayProgress.goal}
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Revision Progress */}
        {safeTodayProgress.revisionGoal > 0 && (
          <div className="text-center">
            <div className="relative w-24 h-24 mb-2">
              <svg className="w-24 h-24 transform -rotate-90" viewBox="0 0 120 120">
                <circle
                ...
                <motion.circle
                  ...
                  strokeLinecap="round"
                  className={`${
                    safeTodayProgress.revisionAchieved 
                      ? 'text-purple-500' 
                      : 'text-orange-500'
                  }`}
                  initial={{ strokeDasharray: "0 314.16" }}
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  <div className="text-lg font-bold text-gray-900 dark:text-white">
                    {safeTodayProgress.revised}
                  </div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    of {safeTodayProgress.revisionGoal}
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="mt-4 p-3 bg-gradient-to-r from-primary-50 to-primary-100 dark:from-primary-900 dark:to-primary-800 rounded-lg">
        <div className="flex items-center space-x-2">
          ...
          <p className="text-sm text-primary-700 dark:text-primary-300">
            {safeTodayProgress.achieved && safeTodayProgress.revisionAchieved && safeTodayProgress.revisionGoal > 0
              ...
              : `${safeTodayProgress.goal - safeTodayProgress.solved} more to go! You've got this! 💪`
            }
          </p>
        </div>
      </div>
