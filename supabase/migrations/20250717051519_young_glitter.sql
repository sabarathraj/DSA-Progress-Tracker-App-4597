/*
  # Comprehensive DSA Tracker Schema

  1. New Tables
    - `user_profiles` - Enhanced user profiles with revision preferences
    - `problems` - Comprehensive problem management with metadata
    - `problem_tags` - Flexible tagging system
    - `user_problems` - Advanced progress tracking with confidence levels
    - `code_snippets` - Version-controlled code storage
    - `daily_progress` - Enhanced daily tracking with revision goals
    - `user_badges` - Achievement system
    - `revision_sessions` - Interview preparation tracking
...
CREATE INDEX IF NOT EXISTS idx_user_problems_next_revision ON user_problems(next_revision_date) WHERE next_revision_date IS NOT NULL;
